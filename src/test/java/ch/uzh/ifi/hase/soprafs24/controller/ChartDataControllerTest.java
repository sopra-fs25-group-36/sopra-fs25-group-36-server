...
public class ChartDataControllerTest {
    ...
    @BeforeEach
    void setUp() {
        // Reset mocks before each test if needed (often handled by Spring automatically)
    ...
    public void whenPostFetchDailyAndServiceThrowsException_thenStatusInternalServerError() throws Exception {
        ...
        doThrow(new RuntimeException(errorMessage)).when(chartDataService).fetchAndStoreDailyDataForAllSymbols();
    ...
    @Test
    public void whenGetDailyChartDataWithValidSymbolAndNoDataExists_thenReturnEmptyListAndStatusOK() throws Exception {
     ...
     public void whenGetDailyChartDataWithCaseInsensitiveSymbol_thenUsesUppercaseAndReturnsData() throws Exception {
         ...
         List<StockDataPointDTO> dataList = Collections.singletonList(testDataPointDTO); // testDataPointDTO is AAPL
         given(chartDataService.getDailyChartData("AAPL")).willReturn(dataList);
         given(chartDataService.getDailyChartData("aapl")).willReturn(Collections.emptyList()); // Mock lowercase call if needed, though controller should uppercase

         // Act: Perform the GET request with lowercase symbol
         MockHttpServletRequestBuilder getRequest = get("/api/charts/aapl/daily")
                 .accept(MediaType.APPLICATION_JSON);

         // Assert: Should still find data because controller uppercases before calling service
         mockMvc.perform(getRequest)
