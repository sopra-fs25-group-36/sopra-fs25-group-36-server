...
class NewsControllerTest {
    ...
    void fetchNewsManually_success() throws Exception {
        ...
        mockMvc.perform(post("/api/news/fetch")
                ...
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().string("News fetching process initiated for tickers: AAPL,MSFT between 2024-01-01 and 2024-01-31"));
    }
    ...
    void fetchNewsManually_singleTicker() throws Exception {
        ...
        mockMvc.perform(post("/api/news/fetch")
                ...
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().string("News fetching process initiated for tickers: AAPL between 2024-01-01 and 2024-01-31"));
    }
    ...
    void fetchNewsManually_invalidStartDateFormat() throws Exception {
        mockMvc.perform(post("/api/news/fetch")
                ...
                .andExpect(content().string(org.hamcrest.Matchers.containsString("Invalid date format. Please use YYYY-MM-DD. Error:")));
    }
    ...
    void fetchNewsManually_emptyTickersList() throws Exception {
        ...
        // Test with empty tickers parameter (though Spring might handle this differently)
        mockMvc.perform(post("/api/news/fetch")
                .param("tickers", "")
                .param("startDate", startDate)
                .param("endDate", endDate)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk()); // Depending on your service logic, this might be different
    }
    ...
    void fetchNewsManually_futureDate() throws Exception {
        ...
        mockMvc.perform(post("/api/news/fetch")
                ...
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().string("News fetching process initiated for tickers: AAPL between 2025-01-01 and 2025-01-31"));
    }
    ...
    void fetchNewsManually_startDateAfterEndDate() throws Exception {
        ...
        // This test assumes the service will handle this validation
        // If validation is done at controller level, you might want to add that logic
        doNothing().when(newsService).fetchAndSaveNewsForTickers(
            anyList(), 
            eq(LocalDate.parse(startDate)), 
            eq(LocalDate.parse(endDate))
        ...
        mockMvc.perform(post("/api/news/fetch")
                ...
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().string("News fetching process initiated for tickers: AAPL between 2024-01-31 and 2024-01-01"));
    }
}